...
export const listProducts = ({
  ...
  try {
    const { data } = await Axios.get(
      `/controller/api/products?seller=${seller}&name=${name}&category=${category}&min=${min}&max=${max}&rating=${rating}`
    );
...
export const listProductCategories = () => async (dispatch) => {
  ...
  try {
    const { data } = await Axios.get(`/controller/api/products/categories`);
...
export const updateProduct = (product) => async (dispatch, getState) => {
  ...
  try {
    const { data } = await Axios.put(`/controller/api/products/${product._id}`, product, {
      headers: { Authorization: `Bearer ${userInfo.token}` },
    });
...
export const deleteProduct = (productId) => async (dispatch, getState) => {
  ...
  try {
    const { data } = Axios.delete(`/controller/api/products/${productId}`, {
      headers: { Authorization: `Bearer ${userInfo.token}` },
    });
